...
interface Props {
  /**
   * City used for the "Popular near you" row. Derived server-side from
   * Vercel geolocation headers so the homepage can personalize without
   * hydration mismatches. Falls back to "San Francisco" when the request
   * is outside California (or when headers aren't available, e.g. local).
   */
  popularCity: string;
}

export default function HomePageClient({ popularCity }: Props) {
    ...
    <div className="bg-white">
      ...
      <section>
        <div className="page-container pt-12 sm:pt-16 pb-8 sm:pb-10">
          ...
          <PopularNearYou city={popularCity} />
        </div>
      </section>
...
/**
 * "Popular near you" — a row of curated, city-aware deep links into
 * /search. The city is determined server-side from Vercel geolocation
 * headers (see app/page.tsx). Keeps the homepage relevant without a
 * dedicated city switcher.
 */
  ...
  const links: Array<{ href: string; label: string }> = [
    {
      ...
      label: `Established plumbers near ${city}`,
    },
    {
      href: `/search?location=${cityParam}&trade=roofing`,
      label: `Roofers near ${city}`,
    },
    {
      href: `/search?location=${cityParam}&trade=electrical`,
      label: `Electricians near ${city}`,
    },
  ];

  return (
    <div className="mt-5 flex flex-wrap items-center justify-center gap-x-3 gap-y-2 text-[13px]">
      <span className="text-ink-tertiary">Popular near you:</span>
      {links.map((l) => (
        <Link
          key={l.href}
          href={l.href}
          className="inline-flex items-center rounded-full border border-line-subtle bg-white px-3 py-1 text-ink-secondary hover:border-ink-hero hover:text-ink-hero focus-brand transition-colors"
        >
          {l.label}
        </Link>
      ))}
